...
flags.DEFINE_string("task_name", 'Muti_Label_Cail2019', "The name of the task to train.")
...
flags.DEFINE_float("num_train_epochs", 32,
...
class Muti_Label_Cail2019(DataProcessor):
    """Processor for the MultiNLI data set (GLUE version)."""

    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "input.json")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "input.json")),"dev_matched")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "input.json")), "test")

    def get_labels(self):
        """See base class."""
        return ['DV1', 'DV2', 'DV3', 'DV4', 'DV5', 'DV6', 'DV7', 'DV8', 'DV9', 'DV10', 'DV11', 'DV12', 'DV13','DV14', 'DV15', 'DV16', 'DV17', 'DV18', 'DV19', 'DV20']
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(per_example_loss, label_ids, logits, is_real_example):
        ...
        predictions = tf.cast(predictions, tf.float32) #类型转换
        one_hot_prediction = tf.cast(tf.greater(predictions, threshold), tf.int64)
...
def result_labels():
  ...
  processors = {
      "muti_label_cail2019": Muti_Label_Cail2019,
...
def generate_pred_file(tags_list, inf_path, outf_path,jb_class):
    with open(inf_path, 'r',encoding='utf-8') as inf,open(outf_path, 'w', encoding='utf-8') as outf:
        pred_labels = result_labels()
        sum = 0
        count = 0
...
if __name__ == "__main__":
  ...
  # divorce领域预测
  tags_list = []
  inf_path = '/home/yangweizhe/法研杯/sentiment_analysis_fine_grain-master/data/TEXT_DIR/input.json'
  outf_path = '/home/yangweizhe/法研杯/sentiment_analysis_fine_grain-master/data/TEXT_DIR/output_divorce_server.json'
  with open('/home/yangweizhe/法研杯/sentiment_analysis_fine_grain-master/data/TEXT_DIR/tags.txt', 'r', encoding='utf-8') as tagf:
      for line in tagf.readlines():
          tags_list.append(line.strip())
  generate_pred_file(tags_list, inf_path, outf_path, 'divorce')
