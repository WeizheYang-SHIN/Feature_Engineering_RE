# -*- coding:utf-8 -*-
# @Time : 2019/9/22 17:02
# @Author : YingHu
# @Email : 977568579@qq.com
# coding=utf-8
...
FLAGS = flags.FLAGS
bert_path = '/home/huying/bio-bert'
...
flags.DEFINE_string("task_name", 'Boundary', "The name of the task to train.")
...
flags.DEFINE_string(
    "init_checkpoint", bert_path + '/model.ckpt-1000000',
...
class DataProcessor(object):
    ...
    def _read_tsv(cls, input_file, quotechar=None):
        ...
        with open(input_file, "r", encoding='utf-8') as f:
            lines = []
            sentence_entity_label_all = json.load(f)
            for sentence_entity_label in sentence_entity_label_all:
                line = []
                sentence_a = sentence_entity_label['sentence']
                sentence_b = sentence_entity_label['entity']
                label = sentence_entity_label['label']
                line.append(sentence_a)
                line.append(sentence_b)
                line.append(label)
                lines.append(line)
            return lines


class Boundary_class(DataProcessor):
    ...
    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "dev")), "dev_matched")
    ...
    def get_labels(self):
        """See base class."""
        return ["DNA", "RNA", "cell_line", "cell_type", "protein", "NEG"]
                #["O", "B", "I", "X", "FAC", "GPE", "LOC", "ORG", "PER", "VEH", "WEA", "NEG", "[CLS]", "[SEP]"]
                #["O", "B", "I", "X", "DNA", "RNA", "cell_line", "cell_type", "protein", "NEG", "[CLS]", "[SEP]"] ["O", "B", "I", "X", "protein", "other_name", "DNA", "cell_type", "other_org", "cell_line", "lipid", "virus", "multi_cell", "Other","NEG", "[CLS]", "[SEP]"]
...
class CNN_model():
  def __init__(self, embedding, num_classes, embedding_size, filter_sizes, num_filters, l2_reg_lambda=0.0001):
    self.embedding_size = embedding_size
    self.embedding_chars = embedding
    ...
    for i, filter_size in enumerate(filter_sizes):
      with tf.variable_scope('conv-max-pooling-%s' % filter_size):
        conv = tf.layers.conv1d(self.embedding_chars, num_filters, filter_size, name='conv1d')
...
def create_model_cnn(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  cnn = CNN_model(embedding=embedding, num_classes=num_labels,
                    embedding_size=hidden_size, filter_sizes=[5, 7, 9, 11, 13, 15], num_filters=128)
  ...
  with tf.variable_scope("loss"):
      if is_training:
          # I.e., 0.1 dropout
          output_layer = tf.nn.dropout(output_layer, keep_prob=0.96)
...
def performance_on_positives_bert(y_test, y_pred):
    ...
    positive_true_test_num = 5567  # 9705 5567
...
def performance_on_positives_boundry_mul_class(y_test, y_pred):
  ...
  true_index_list = [0, 1, 2, 3, 4]
  # type_list=["FAC", "GPE", "LOC", "ORG", "PER", "VEH", "WEA"]
  type_list = ["DNA", "RNA", "cell_line", "cell_type", "protein"]
  # type_list = ["protein", "other_name", "DNA", "cell_type", "other_org", "cell_line", "lipid", "virus", "multi_cell", "Other"]

  # list_true_test_num = [83, 911, 81, 467, 1847, 34, 54]#测试集中的正例
  list_true_test_num = [1274, 111, 461, 614, 3107]#GENIA测试集中的正例
  # list_true_test_num = [3141, 2266, 1301, 630, 352, 481, 351, 301, 284, 598]  # GENIA测试集中的正例
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        true_label = []
        prediction_label = []
        true_label_mutilabel = []
        prediction_label_mutilabel = []

        for y_true, y_prediction in zip(predict_examples, result):
            #多分类
            true_label_mutilabel.append(label_list.index(y_true.label))
            prediction_label_mutilabel.append(np.argmax(y_prediction['probabilities']))
            #二分类
            if label_list.index(y_true.label) == 5:
                true_label.append(1)
            else:
                true_label.append(0)
            if np.argmax(y_prediction['probabilities']) == 5:
                prediction_label.append(1)
            else:
                prediction_label.append(0)
        performance_on_positives_bert(true_label, prediction_label)  #二分类
        performance_on_positives_boundry_mul_class(true_label_mutilabel, prediction_label_mutilabel)  #多分类
